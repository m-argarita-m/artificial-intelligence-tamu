...
class Sexpr:
    def __init__(self, toks, i):
        ...
        if toks[i] == '(':
            ...
            self.next = i; return  # this is not my close paren; parent might be interested in it
        else:
            self.atom = toks[i]; self.next = i + 1; return
...
def push_negs_inward(expr):
    ...
    if expr.list[0].atom == 'not':  # assume each list has at least 1 item, the oper, which is an atom
        ...
        if oper.atom == "and" or oper.atom == "or":
            ...
            raise Exception("error: don't know how to push negation inward over the following operator: %s" % oper.atom)
    else:
    ...
    if oper == "and":
        ...
        for i in range(len(args)):
            ...
            if is_conjunction(arg):
                ...
                return distribute(make_Sexpr_from_list(
                    ["and"] + newargs))  # after processing this arg, use recursion to handle any others
...
if __name__ == "__main__":
    ...
    for line in open(sys.argv[1]):
        ...
        if is_conjunction(cnf):
            ...
            clauses = [cnf]
        clauses = [or_wrapper(c) for c in
                   clauses]  # make sure clauses have an 'or' at the top level; (should not have 'and's at top level)
